...
//			,27 // position
		...
		add_submenu_page(
			$this->plugin_name,
			__( 'SoaVis Shortcodes', 'wp-soavis' ),
			__( 'SoaVis Shortcodes', 'wp-soavis' ),
			'edit_posts',
	...
	public function render_soavis_settings_section() {
		echo _e( 'Manage your SoaVis settings below.', 'wp-soavis' );
	}
	...
	public function render_wp_soavis_max_graph_level_field() {
		...
		<?php
		echo ' ' . __('The maximum level to traverse the service network.', 'wp-soavis') . ' ';
		echo sprintf(__('Must be a positive integer, any invalid value will render to the default value of %d.', 'wp-soavis'), WP_SOAVIS_MAX_GRAPH_LEVEL_DEFAULT) . ' ';
	}
	...
	function render_soavis_shortcode_page() {
		...
		<div class="wrap">
			<?php screen_icon(); ?>
			<h3><?php echo __('Shortcodes', 'wp-soavis'); ?></h3>
			<?php
			if (isset($_REQUEST['settings-updated'])) {
				?>
				<div id="sip-return-message" class="updated"><?php echo __('Your Settings have been saved.', 'wp-soavis'); ?></div>
				<?php
			}
			?>
			<p>
				<?php echo __('This page shows the shortcodes supported by the WP SoaVis plugin:', 'wp-soavis'); ?>
			</p>
			...
			<thead>
				<tr class="wp_soavis_shortcodes_row">
					...
					<th class="wp_soavis_shortcodes_cell" width="25%"><?php echo __('Description', 'wp-soavis'); ?>&nbsp;</th>
					<th class="wp_soavis_shortcodes_cell"><?php echo __('Parameters', 'wp-soavis'); ?>&nbsp;</th>
				</tr>
			</thead>
			<tbody>
				<?php
		...
		<p>
			<?php
				echo sprintf (__('<br/>This plugin supports the shortcodes as shown above as well as in all upper and lower case.', 'wp-soavis'));
				echo sprintf (__('<br/>More information can be found <a href="%s">here</a>.', 'wp-soavis'), WP_SOAVIS_LINK);
				echo sprintf (__('<br/>If you have suggestions to improve this plugin, please leave a comment on this same <a href="%s">page</a>.', 'wp-soavis'), WP_SOAVIS_LINK);
			?>
		</p>
...
//		$this->debugMP('pr', __FUNCTION__ . ' in_array: [' . in_array($column_name, $this->plugin->wps_post_types->info_box_params) . '] , info_box_params', $this->plugin->wps_post_types->info_box_params[$column_name]);
	...
	public function add_soavis_information_meta_box( $post ) {
		$this->debugMP('pr', __FUNCTION__ . ' REQUEST:', $_REQUEST);
		$post_type = (isset($_REQUEST['post_type'])) ? $_REQUEST['post_type'] : 'soavis_service';
		add_meta_box(
			'soavis-information-meta-box',
			__( 'SoaVis Information', 'wp-soavis' ),
	...
	public function wps_action_media_buttons(){
		...
		$shortcodes_list .= '&nbsp;<select id="sc_select"><option>' . __('SoaVis Shortcode', 'wp-soavis') . '</option>';
